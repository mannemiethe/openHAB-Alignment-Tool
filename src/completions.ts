...
function getIconSourceCompletions(): vscode.CompletionItem[] {
	return [
		keyword({
			label: "oh:",
			insertText: "oh:${1:water}",
			detail: "openHAB icon source prefix",
			documentation: "Explicit openHAB classic icon reference, e.g. `<oh:water>`. A plain `<water>` is equivalent.",
			kind: vscode.CompletionItemKind.Module,
		}),
		keyword({
			label: "material:",
			insertText: "material:${1:favorite}",
			detail: "Material icon source prefix",
			documentation: "Material icon reference, e.g. `<material:favorite>`.",
			kind: vscode.CompletionItemKind.Module,
		}),
		keyword({
			label: "f7:",
			insertText: "f7:${1:house}",
			detail: "Framework7 icon source prefix",
			documentation: "Framework7 icon reference, e.g. `<f7:house>`.",
			kind: vscode.CompletionItemKind.Module,
		}),
		keyword({
			label: "if:",
			insertText: "if:${1:mdi}:${2:water}",
			detail: "Iconify icon source prefix",
			documentation: "Iconify icon reference, e.g. `<if:mdi:water>`. Requires client internet connectivity where openHAB UI supports Iconify.",
			kind: vscode.CompletionItemKind.Module,
		}),
		keyword({
			label: "iconify:",
			insertText: "iconify:${1:mdi}:${2:water}",
			detail: "Iconify icon source prefix",
			documentation: "Long Iconify source alias, e.g. `<iconify:mdi:water>`.",
			kind: vscode.CompletionItemKind.Module,
		}),
	];
}
...
function getIconCompletions(): vscode.CompletionItem[] {
	return [
		...getIconSourceCompletions(),
		...classicIcons.map((label) => keyword({
			label,
			insertText: label,
			detail: "openHAB classic icon",
			documentation: `Classic openHAB icon. Use as \`<${label}>\` or explicitly as \`<oh:${label}>\`.`,
...
function isInsideItemIconReference(linePrefix: string): boolean {
	let lastOpen = linePrefix.lastIndexOf("<");
	let lastClose = linePrefix.lastIndexOf(">");
	if (lastOpen <= lastClose) {
		return false;
	}
	let beforeIcon = linePrefix.substring(0, lastOpen);
	return /^\s*(?:Call|Color|Contact|DateTime|Dimmer|Group(?::\S+)?|Image|Location|Number(?::\S+)?|Player|Rollershutter|String|Switch)\s+\S+(?:\s+"(?:\\.|[^"])*")?\s*$/.test(beforeIcon);
}
...
function getContextualItemCompletions(document: vscode.TextDocument, position: vscode.Position): vscode.CompletionItem[] | undefined {
	...
	if (isInsideItemIconReference(linePrefix)) {
